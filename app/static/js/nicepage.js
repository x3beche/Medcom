				...
				submit: function(i) {
					...
					o.ajax({
						...
						if (data && (data.success || data.ok || "success" === t)){
                            console.log(t);
                            document.getElementById("success_message").textContent=data.info;
                            if (FormMessage.showSuccess(form), a) window.location.replace(a);
							else e(form);

							if(data.info == "Login Success"){ setTimeout(function () { window.location.href = "/"; }, 1000); }
							
							if (data.info == "Successfuly registered.") { document.getElementById("event_button").textContent = "Cancel Registration"; }
							...
							if(data.info == "Event found."){
							setTimeout(function() {  document.getElementById("success_message").textContent="We direct you to the event page.";  }, 2000);
                            setTimeout(function() {  window.location.href = "/dashboard/community_event/"+data.event_id; }, 4000);}
						}
						else FormMessage.showError(form, data.error)
					})).fail((function(data, t) {
                        document.getElementById("error_message").textContent=data.responseJSON.error;
						FormMessage.showError(form);

						if(data.responseJSON.error == "Registration canceled."){ document.getElementById("event_button").textContent = "Register Event"; }
